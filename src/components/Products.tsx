...
export const Products: React.FC = () => {
  ...
  // Categories list
  const customCategories = ['مواد بهداشتی و آرایشی', 'نوشیدنی‌ها', 'خواربار و مواد غذایی'];
  const categoriesList = ['All', ...Array.from(new Set([...customCategories, ...state.products.map(p => p.category)]))];
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden relative">
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <h4 className="text-[#D4AF37] font-bold border-b border-slate-100 pb-2">اطلاعات اولیه</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-600 mb-1">نام محصول</label>
                    <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37]" placeholder="مثال: شامپو صحت" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-slate-50" placeholder="مثال: قفسه A-12" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-600 mb-1">قیمت خرید (میانگین)</label>
                    <input type="number" step="0.01" required value={formData.costPriceUSD} onChange={e => setFormData({...formData, costPriceUSD: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-slate-50 font-mono text-left" dir="ltr" />
                  </div>
                  <div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  <div className="col-span-1 md:col-span-1">
                    ...
                    <input type="text" required value={formData.baseUnit} onChange={e => setFormData({...formData, baseUnit: e.target.value})} className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-slate-50" placeholder="دانه" />
                  </div>
                  <div className="col-span-1 md:col-span-3 flex flex-wrap gap-4">
                    <label className="flex items-center gap-2 text-sm font-bold text-slate-600 bg-slate-100 px-3 py-2 rounded-lg cursor-pointer">
